...
if kcnt >= ihopsize then
 ...
 kSpec[] rfft kWin ;FFT
   ...
   ;INVERSIONE BANDE
   iFreq = 500 ;CENTRO DI INVERSIONE
   iBandeInf = int(iFreq / ibw) ;NUMERO DI BANDE INFERIORE
   iBandeSup = int((sr - iFreq) / ibw) ;NUMERO DI BANDE SUPERIORE
   iOnOffBandInf = 0 ;FILTRO BANDE INFERIORI
   iOnOffBandSup = 1 ;FILTRO BANDE SUPERIORI
   
   ;CICLO PER IL CALCOLO DELL'INVERSIONE
   ki = 0 
   until ki == int(ifftsize) do
   
   if ki < iBandeInf then
   kSpec[ki] = kSpec[int(ki * iBandeInf / ifftsize)] * iOnOffBandInf
   else
   kSpec[ki] = kSpec[iBandeInf + int(ki * iBandeSup / ifftsize)] * iOnOffBandSup
   endif

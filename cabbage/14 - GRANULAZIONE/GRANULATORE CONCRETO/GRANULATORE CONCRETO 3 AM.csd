<Cabbage> bounds(0, 0, 0, 0)
    form caption("GRANULATORE CONCRETO AM") size(850, 400), pluginId("def1"), colour("40,40,40,255")
    ...
    label bounds(450, 256, 102, 18) channel("label10025") text("ENVELOPE")
    combobox bounds(450, 278, 102, 20) channel("func"), text("Hanning", "Bartlett (Triangle)", "Gaussian")
    ...
    rslider bounds(140, 328, 60, 60) channel("AMcar") range(0, 1, 0, 1, 0.001) text("AM CAR")
    rslider bounds(202, 328, 60, 60) channel("AMmod") range(0, 1000, 0, 1, 0.001) text("AM MOD")
    rslider bounds(264, 328, 60, 60) channel("Spread") range(0, 1, 0, 1, 0.001) text("AM Spread")
...
<CsoundSynthesizer>
    ...
    instr 1
        ...
        ;AM
        gkAMcar  cabbageGetValue "AMcar"
        gkAMmod  cabbageGetValue "AMmod"
        gkSpread  cabbageGetValue "Spread"
    ...
    instr 3 
        ...
        ;AM
         aMod oscili i(gkAMcar), i(gkAMmod) + rnd(i(gkAMmod) * i(gkSpread))
         aSigAM = (aSig * aMod) + aSig

        ;SPREAD DI LATERALIZZAZIONE
        irndPan = 0.5 + (((rnd(2) - 1) / 2) * i(gkSpreadPan))

        ;LATERALIZZAZIONE SOURCE
        aL,aR pan2 (aSigAM)  * aenv, irndPan
    ...
    instr 50
        ...
        ;SOMMA USCITE
        aSumL = (aL * gkOut) + (aRevL * gkRev) 
        aSumR = (aR * gkOut) + (aRevR * gkRev) 

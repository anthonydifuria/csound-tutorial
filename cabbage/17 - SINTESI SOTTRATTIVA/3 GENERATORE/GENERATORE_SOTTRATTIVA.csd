<Cabbage>
form caption("GENERATORE SOTTRATTIVA") size(700, 400), guiMode("queue") pluginId("def1")
signaldisplay bounds(78, 164, 362, 187), colour("white") displayType("waveform"), backgroundColour(0, 0, 0), zoom(0.5), signalVariable("aOut", "aOut"), channel("display")
...
rslider bounds(494, 170, 100, 100), channel("WidthBand"), range(2, 400, 100, 1, 0.01), text("WIDTH BAND"), trackerColour(0, 255, 0, 255), outlineColour(0, 0, 0, 50), textColour(0, 0, 0, 255)
...
gkWidthBand cabbageGetValue "WidthBand"
...
;FILTRO
iAmp = (1/i(gkDen))*(i(gkDen)) * 5 ;calcola l'ampiezza rispetto alla durata e densità degli eventi sonori
iFreq = (i(gkFfond) * irndHarm)  + irndSpredFreq ;calcola la frequenza

aPinkNoise pinker ; Generatore di rumore rosa
aSig butterbp aPinkNoise, iFreq, i(gkWidthBand) 

; spread di lateralizzazione
irndPan = 0.5 + (((rnd(2) - 1) / 2) * i(gkSpreadPan))

; lateralizzazione
aL,aR pan2 aSig * aenv * iAmp, irndPan
